...
namespace OSDeveloper.GUIs.Explorer
{
	...
	public partial class FileTreeBoxOld : UserControl
	{
		#region プロパティ
		...
		public FileTreeBoxOld(FormMain mwnd)
		{
			_logger = Logger.Get(nameof(FileTreeBoxOld));
			_mwnd   = mwnd;
			...
			_logger.Trace($"constructed {nameof(FileTreeBoxOld)}");
		}
...
retry:
					...
					try {
						...
						var stn = new SolutionTreeNodeOld(sln);
						stn.Refresh(this);
						_solutions.Add(stn);
					} catch (Exception e) {
		...
		internal void UpdatePItemNode(ProjectItemTreeNodeOld node)
		{
		...
		{
			if (node is ProjectItemTreeNodeOld pitn) {
		...
		private void removeMenu_Click(object sender, EventArgs e)
		{
			...
			if (treeView.SelectedNode is FileTreeNode node) {
				if (node.Metadata.TrashItem()) {
					var p = node.Parent;
					node.Remove();
					(node as ProjectItemTreeNodeOld)?.RemoveItem(node.Metadata);
